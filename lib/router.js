...
function generateRoute(method, name) {
    return function(path) {
        return function(target, key, descriptor) {
            ...
            // 检测重复
            // match(path, {decode: decodeURIComponent})(p) || match(p, {decode: decodeURIComponent})(path)
            const exists = target.__actions__.find(item => {
                return (
                    item.method === method &&
                    (match(item.path, { decode: decodeURIComponent })(_path) ||
                        match(_path, { decode: decodeURIComponent })(item.path))
                )
            })

            if (exists) throw Error(`路由 ${method} ${_path} 与 ${exists.method} ${exists.path} 冲突！`)
